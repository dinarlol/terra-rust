...
pub fn execute_withdrawal_from(
    deps: DepsMut,
    env: Env,
    info: MessageInfo,
    owner: String,
...
mod tests {
    ...
    #[test]
    fn withdrawal_from_respects_limits() {
        ...
        // valid withdrawal of part of the allowance
        let withdrawal = Uint128::from(44444u128);
        let msg = ExecuteMsg::withdrawalFrom {
            owner: owner.clone(),
        ...
        assert_eq!(res.attributes[0], attr("action", "withdrawal_from"));

        // make sure money arrived
        assert_eq!(
            get_balance(deps.as_ref(), owner.clone()),
            start.checked_sub(withdrawal).unwrap()
        );
        assert_eq!(get_balance(deps.as_ref(), rcpt.clone()), withdrawal);

        // ensure it looks good
        let allowance = query_allowance(deps.as_ref(), owner.clone(), spender.clone()).unwrap();
        let expect = AllowanceResponse {
            allowance: allow1.checked_sub(withdrawal).unwrap(),
            expires: Expiration::Never {},
        };
    ...
    fn send_from_respects_limits() {
        ...
        assert_eq!(get_balance(deps.as_ref(), contract.clone()), withdrawal);
